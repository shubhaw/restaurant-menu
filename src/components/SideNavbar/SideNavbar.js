...
const SideNavbar = props => {
    return <div className={props.open ? classes.SideNavbarContainer : classes.Hide}>
        <div className={classes.Overlay}>
            ...
            {props.selectedMenu === "foodMenu" && props.foodMenu.map(item => <div key={item.title} className={classes.NavmenuItem}>{item.title}</div>)}
            <div className={classes.NavmenuItemHeading} onClick={() => props.onMainMenuChange("barMenu")}>Bar Menu</div>
            {props.selectedMenu === "barMenu" && props.barMenu.map(item => <div key={item.title} className={classes.NavmenuItem}>{item.title}</div>)}
        </div>
    </div>
}
